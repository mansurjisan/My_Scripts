...
python plot_maxele.py maxele.63_wcoss2.nc maxele.63_und.nc zeta_max     --region custom --lon-range -85 -65 --lat-range 25 45 --percentage        --vmin -10 --vmax 10   --no-individual --max-points 0 --highlight-extremes      --save mjisan_conus_percent_zeta_max_diff.png
"""
    ...
    for idx, (ax, data, title, cmap) in enumerate(zip(axes, data_list, titles, cmaps)):
        ...
        for ax_current in axes_to_plot:
            ...
            if use_triangulation and len(x_region) < 50000:
                ...
                if norm is not None:
                    im = ax_current.tripcolor(triang, data, shading='gouraud', 
            ...
            # Colorbar
            cbar = plt.colorbar(im, ax=ax_current, shrink=0.8, pad=0.02)
            cbar.ax.tick_params(labelsize=9)
            
            # Labels
            ax_current.set_xlabel('Longitude (degrees)', fontsize=10)
            ax_current.set_ylabel('Latitude (degrees)', fontsize=10)
