...
def plot_difference_regular(nc1, nc2, var_name, time_index1, time_index2, output_file=None,
                           region='global', lon_range=None, lat_range=None,
                           colormap='RdBu_r', dpi=150, vmin=None, vmax=None,
                           color_levels=None, figsize=(12, 8), 
                           smooth=False, add_coastlines=False, symmetric=True):
    """Plot difference between two regular grid files"""
    
    # Detect coordinate systems for both files
    if 'latitude' in nc1.variables and 'longitude' in nc1.variables:
        lat1 = nc1.variables['latitude'][:]
        lon1 = nc1.variables['longitude'][:]
    elif 'grid_yt' in nc1.variables and 'grid_xt' in nc1.variables:
        lat1 = nc1.variables['grid_yt'][:]
        lon1 = nc1.variables['grid_xt'][:]
    
    if 'latitude' in nc2.variables and 'longitude' in nc2.variables:
        lat2 = nc2.variables['latitude'][:]
        lon2 = nc2.variables['longitude'][:]
    elif 'grid_yt' in nc2.variables and 'grid_xt' in nc2.variables:
        lat2 = nc2.variables['grid_yt'][:]
        lon2 = nc2.variables['grid_xt'][:]
    
    # Check if grids match
    if len(lat1) != len(lat2) or len(lon1) != len(lon2):
        print(f"Warning: Grid sizes don't match!")
        print(f"  File 1: {len(lat1)}x{len(lon1)}")
        print(f"  File 2: {len(lat2)}x{len(lon2)}")
    
    # Use first file's grid
    lat = lat1
    lon = lon1
    ...
    if var_name.startswith('WIND_SPEED'):
        # Calculate wind speed for file 1
        level = '10maboveground' if '10m' in var_name else 'surface'
        data1, _ = calculate_wind_speed(nc1, time_index1, level)
        data2, _ = calculate_wind_speed(nc2, time_index2, level)
        
        if data1 is None or data2 is None:
            print("Error: Cannot calculate wind speed for one or both files")
            return None
        ...
        print(f"Direct variable comparison not yet implemented for {var_name}")
        return None
    ...
    # Reorder if needed for longitude wrapping
    if lon_plot[0] > lon_plot[-1]:
        split_idx = np.where(np.diff(lon_plot) < 0)[0]
        if len(split_idx) > 0:
            split_idx = split_idx[0] + 1
    ...
    # Auto-detect if data2 needs to be flipped by comparing patterns
    # Calculate correlation with and without flipping
    data1_flat = data1.flatten()
    data2_flat = data2.flatten()
    data2_flipped = np.flip(data2, axis=0)
    data2_flipped_flat = data2_flipped.flatten()
    
    # Remove NaN values for correlation calculation
    mask = ~(np.isnan(data1_flat) | np.isnan(data2_flat))
    if np.sum(mask) > 100:  # Need enough valid points
        corr_original = np.corrcoef(data1_flat[mask], data2_flat[mask])[0, 1]
        
        mask_flipped = ~(np.isnan(data1_flat) | np.isnan(data2_flipped_flat))
        corr_flipped = np.corrcoef(data1_flat[mask_flipped], data2_flipped_flat[mask_flipped])[0, 1]
        
        print(f"Correlation without flip: {corr_original:.3f}")
        print(f"Correlation with flip: {corr_flipped:.3f}")
        
        if corr_flipped > corr_original + 0.1:  # Significant improvement with flip
            print("Auto-detected flipped latitude orientation. Applying correction...")
            data2 = data2_flipped
        elif abs(corr_flipped - corr_original) < 0.1:
            print("Warning: Cannot determine orientation with confidence. Using original.")
        else:
            print("Data appears correctly oriented.")
    ...
    # Get time strings for both files
    def get_time_string(nc, time_idx):
        if 'time' in nc.variables:
            time_var = nc.variables['time']
            if hasattr(time_var, 'shape') and len(time_var.shape) == 0:
                # Scalar time - calculate from index
                base_time_val = time_var[:]
                actual_time_val = base_time_val + time_idx
            else:
                actual_time_val = time_var[time_idx] if len(time_var.shape) > 0 else time_var[:]
            
            if hasattr(time_var, 'units'):
                from netCDF4 import num2date
                time_obj = num2date(actual_time_val, time_var.units)
                return time_obj.strftime('%H%M UTC %b %d, %Y')
        return f"Index {time_idx}"
    
    time_str1 = get_time_string(nc1, time_index1)
    time_str2 = get_time_string(nc2, time_index2)
    ...
    # Calculate statistics
    diff_masked = np.ma.masked_invalid(diff_data)
    diff_min, diff_max = diff_masked.min(), diff_masked.max()
    diff_mean = diff_masked.mean()
    diff_std = diff_masked.std()
    diff_rms = np.sqrt((diff_masked**2).mean())
    
    print(f"\nDifference Statistics for {var_name}:")
    print(f"  File 1 ({time_str1}) mean: {np.ma.masked_invalid(data1).mean():.3f}")
    print(f"  File 2 ({time_str2}) mean: {np.ma.masked_invalid(data2).mean():.3f}")
    print(f"  Difference (File2 - File1):")
    print(f"    Min: {diff_min:.3f}, Max: {diff_max:.3f}")
    print(f"    Mean: {diff_mean:.3f}, Std: {diff_std:.3f}, RMS: {diff_rms:.3f}")
    ...
    if use_cartopy:
        im = ax.pcolormesh(lon_grid, lat_grid, diff_data,
                          cmap=colormap, shading='auto',
                          vmin=plot_vmin, vmax=plot_vmax,
                          transform=ccrs.PlateCarree())
    else:
    ...
    cbar = plt.colorbar(im, ax=ax, shrink=0.8, pad=0.02)
    cbar.ax.tick_params(labelsize=9)
    cbar.set_label(f'Wind Speed Difference ({units})', fontsize=10)
    ...
    # Add custom title elements
    ax.text(0.02, 1.02, f'{time_str1} - {time_str2}', transform=ax.transAxes,
            fontsize=11, fontweight='bold', verticalalignment='bottom', horizontalalignment='left')
    
#    ax.text(0.5, 1.02, f'{long_name} Difference', transform=ax.transAxes,
#            fontsize=11, fontweight='bold', verticalalignment='bottom', horizontalalignment='center')
    
    ax.text(0.98, 1.02, f'Min | Max  {diff_min:.1f} | {diff_max:.1f}', transform=ax.transAxes,
            fontsize=11, fontweight='bold', verticalalignment='bottom', horizontalalignment='right')
    ...
    if output_file:
        fig.savefig(output_file, dpi=300, bbox_inches='tight',
                   facecolor='white', edgecolor='none')
        print(f"\nDifference plot saved to: {output_file}")
    else:
        plt.show()
...
def main():
    ...
    parser.add_argument('--time1', type=int, default=None,
                        help='Time index for first file (difference mode)')
    parser.add_argument('--time2', type=int, default=None,
                        help='Time index for second file (difference mode)')
    ...
    # Handle difference mode
    if args.mode == 'difference' or args.file2 is not None:
        if args.file2 is None:
            print("Error: Difference mode requires --file2")
            sys.exit(1)
        
        nc1, file_type1 = load_netcdf_data(args.file1)
        nc2, file_type2 = load_netcdf_data(args.file2)
        
        if file_type1 != file_type2:
            print(f"Warning: File types differ ({file_type1} vs {file_type2})")
        
        # Set time indices
        time1 = args.time1 if args.time1 is not None else args.time
        time2 = args.time2 if args.time2 is not None else args.time
        print(f"\nComparing {args.file1} (t={time1}) with {args.file2} (t={time2})")
        
        # Get variable if not specified
        if args.variable is None:
            args.variable = 'WIND_SPEED_10m'
        ...
        plot_kwargs = {
            ...
            'colormap': args.colormap if args.colormap != 'viridis' else 'RdBu_r',
            'dpi': args.dpi,
            'vmin': args.vmin,
            'vmax': args.vmax,
        ...
        # Plot difference
        if file_type1 == 'regular':
            plot_difference_regular(nc1, nc2, args.variable, time1, time2,
                                  output_file=args.save, **plot_kwargs)
        else:
            print("Unstructured difference plotting not yet implemented")
        
        nc1.close()
        nc2.close()
        return
    
    # Original single file mode code continues below
